...
def plot_ks(y_test, y_score, positive_flag):
    ...
    plt.plot([0]+cuts.tolist()+[1], [0]+plot_data.y2.tolist()+[1], label='Sensitivity')
     # 添加参考线
    plt.vlines(plot_data.cuts[max_ks_index], ymin=plot_data.y1[max_ks_index],ymax=plot_data.y2[max_ks_index], linestyles='--')
    # 添加文本信息
    plt.text(x=plot_data.cuts[max_ks_index]+0.01,y=plot_data.y1[max_ks_index]+plot_data.ks[max_ks_index]/2,s='KS= %.2f' %plot_data.ks[max_ks_index])
    # 显示图例
    plt.legend()
    # 显示图形
    plt.show()
...
# 导入虚拟数据
path1 = r'E:\workspace\testdatas\virtual_data.xlsx'
virtual_data = pd.read_excel(path1)
# 应用自定义函数绘制k-s曲线
plot_ks(y_test=virtual_data.Class, y_score=virtual_data.Score,positive_flag='P')

# 读取数据
path2 = r'E:\workspace\testdatas\Run or Walk.csv'
sports = pd.read_csv(path2)
# 提取出所有自变量名称
predictors = sports.columns[4:]
# print(predictors)
# 构建自变量矩阵，ix[:'行标签/行索引'，:'列数值/列的位置']用于截取矩阵中想要的行和列形成子矩阵;现在使用iloc[:df.index.get_loc('c') + 1, :4]
X = sports[predictors]
# 提取y变量值
y = sports.activity
# 将数据集拆分为训练集和测试集
...
# -----------------------第三步 绘制ROC曲线 ----------------------- #
    # 计算真正率和假正率 \n",pos_label=2
fpr,tpr,thresholds = metrics.roc_curve(y_test, sm_y_probability)
    # 计算auc的值
roc_auc = metrics.auc(fpr,tpr)
    # 绘制面积图\n",
plt.stackplot(fpr, tpr, color='steelblue', alpha=0.5, edgecolor='black')
