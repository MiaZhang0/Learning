'''
早上上班场景：为了不迟到，总得提前几分钟到办公室，打卡之后，一看表，还不到工作时间，大家就看看新闻、聊聊天啥的；
工作时间一到，立马开工。如果有人迟到了呢，自然就不能看新闻聊天了，得立即投入工作中。
这个场景中，每个人代表一个线程，工作时间到，表示事件（event）发生。
事件发生前，线程会调用wait()方法阻塞自己（对应看新闻聊天），一旦事件发生，会唤醒所有调用wait()而进入阻塞状态的线程。
'''
# -*- encoding: utf8 -*-
